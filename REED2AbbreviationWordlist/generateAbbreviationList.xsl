...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:reed="http://reed.utoronto.ca/ns/abbrev-stylesheet"
    xmlns="http://www.tei-c.org/ns/1.0"
    xpath-default-namespace="http://www.tei-c.org/ns/1.0"
    exclude-result-prefixes="xs map xd tei reed"
    version="3.0">
    ...
    <xd:doc scope="stylesheet">
        <xd:desc>
            ...
            <xd:p><xd:b>Rewritten:</xd:b> more recently as XSLT 3.0, using map()-based grouping.</xd:p>
            <xd:p>Convert REED Volumes to a TEI-encoded dictionary of scribal abbreviations.</xd:p>
            <xd:p>Run with (Saxon 9.8 HE or later; requires XPath 3.1 map support):
                rm abbreviations-wordlist.xml ;
                saxon -xsl:generateAbbreviationList.xsl -it:main -o:abbreviations-wordlist.xml
                    [files=some-glob.xml] [recurse=yes] [debug=false]
                    [include-editorial-notes=true] [min-frequency=2] [sort-by=frequency]
            </xd:p>
        </xd:desc>
    </xd:doc>
    ...
    <xsl:template name="main">
        <xsl:if test="$debug">
            <xsl:message select="concat('[reed-abbrev] loading documents matching &quot;', $files,
                '&quot; (recurse=', $recurse, ')')"/>
        ...
        <xsl:variable name="languages" select="distinct-values($abbrev-instances/@xml:lang)" as="xs:string*"/>

        <xsl:if test="$debug">
            <xsl:message select="concat('[reed-abbrev] ', count($merged),
                ' orthographic word-tokens assembled; ', count($abbrev-instances),
                ' contain at least one editorially-supplied abbreviation')"/>
            <xsl:message select="concat('[reed-abbrev] languages found: ', string-join($languages, ', '))"/>
            <xsl:if test="count($abbrev-instances[@xml:lang = $default-lang]) gt 0">
                <xsl:message select="concat('[reed-abbrev] WARNING: ',
        ...
        <xsl:variable name="lang-indexes" as="map(xs:string, map(xs:string, map(*)))" select="
            map:merge(
                for $l in $languages
                return map{ $l : reed:build-index($abbrev-instances[@xml:lang = $l]) }
            )"/>

        <xsl:variable name="total-distinct-forms" as="xs:integer"
            select="sum(for $l in $languages return map:size(map:get($lang-indexes, $l)))"/>
        ...
        <TEI xmlns="http://www.tei-c.org/ns/1.0">
            <teiHeader>
                <fileDesc>
                    ...
                    <sourceDesc>
                        <p>Automatically extracted from the abbreviated word-forms found in the
                            ...
                            document(s), listed below by their declared title (falling back to
                            the file's URI if no title could be found).
                        <listBibl>
                ...
                <encodingDesc>
                    <editorialDecl>
                        <p>Word forms are drawn only from div[type="transcription"] elements in
                            each source document.</p>
                        <p>By default, content inside note[type="foot"] (editorial footnotes) is
                            ...
                            include-editorial-notes=<xsl:value-of select="$include-editorial-notes"/>.</p>
                        ...
                        <p>Distinct entries are determined by the exact shape of the word,
                            i.e. by both its letters and the position(s) of the editorially
                            supplied portion(s); two words that look identical once expanded but
                            differ in what was actually written by the scribe are listed as
                            separate entries.</p>
                    </editorialDecl>
                </encodingDesc>
                <profileDesc>
                    <langUsage>
                        <xsl:for-each select="$languages">
                            <xsl:sort select="."/>
                            <language ident="{.}"><xsl:value-of select="reed:lang-label(.)"/></language>
                        </xsl:for-each>
                    </langUsage>
                </profileDesc>
                <revisionDesc>
                    <change when="{format-date(current-date(), '[Y0001]-[M01]-[D01]')}">
                        This version of the file was generated. 
                    </change>
                </revisionDesc>
            </teiHeader>
            <text>
                <body>
                    ...
                    <p>This is an automatically generated index of scribal abbreviations found in
                        the transcribed text of the source documents listed above.
                        <xsl:value-of select="count($abbrev-instances)"/> total abbreviated-word
                        instances were found, representing <xsl:value-of select="$total-distinct-forms"/>
                        distinct abbreviated forms, split below by language.</p>
                    <p>Each entry gives the attested form (with editorially supplied letters
                        marked by <mentioned>ex</mentioned>), its frequency, and a citation back to
                        the <mentioned>xml:id</mentioned> of the manuscript transcription division
                        in which each instance occurs, so any attestation can be checked against
                        its source.</p>
                    ...
                    <xsl:for-each select="$languages">
                        ...
                        <xsl:if test="$debug">
                            <xsl:message select="concat('[reed-abbrev] language &quot;', $lang, '&quot; (',
    ...
    <xsl:template match="note[@type = 'foot']" mode="tokenize">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:if test="$debug">
                    <xsl:message select="concat('[reed-abbrev] skipping editorial footnote in ',
        ...
        <xsl:for-each-group select="$tokens" group-adjacent="boolean(self::text())">
            <xsl:if test="not(current-grouping-key())">
                ...
                <w xml:lang="{$first/@xml:lang}" div="{$first/@div}">
                    <xsl:if test="some $g in current-group() satisfies $g/@supplied = 'true'">
                        <xsl:attribute name="supplied">true</xsl:attribute>
                    </xsl:if>
                    <xsl:for-each select="current-group()">
                        <xsl:choose>
                            <xsl:when test="self::ex"><ex><xsl:value-of select="."/></ex></xsl:when>
    ...
    <xsl:template name="dictionary-division">
        ...
        <div type="dictionary" xml:lang="{$lang}">
            ...
            <xsl:for-each select="map:keys($index)[xs:integer(map:get($index, .)?count) ge $min-frequency]">
                <!-- Primary sort by frequency when requested; defaults to 0 otherwise -->
                <xsl:sort select="if ($sort-by = 'frequency') then xs:integer(map:get($index, .)?count) else 0"
                    data-type="number"
                    order="descending"/>
                <!-- Secondary/Alphabetical sort by stripped word form -->
                <xsl:sort select="lower-case(reed:sort-text(.))"
                    data-type="text"
                    order="ascending"/>
                ...
                <entry xml:id="{reed:entry-id($lang, position())}" xml:lang="{$lang}">
                    ...
                    <form type="attestation">
                        <orth><xsl:copy-of select="$v?sample/node()"/></orth>
                    </form>
                    <usg type="frequency"><xsl:value-of select="$v?count"/></usg>
              
            <!-- Single cit block containing the quoted form once, followed by all instance refs -->
            <cit type="examples">
                <quote><xsl:copy-of xml:space="preserve" select="$v?sample/node()"/></quote>
                <xsl:for-each select="$v?instances">
                    <xsl:choose>
                        <xsl:when test="string-length(@div) gt 0">
                            <ref target="{concat('https://ereed.org/records/', substring-before(@div, '-transcription'))}"><xsl:value-of select="substring-before(@div, '-transcription')"/></ref>
                        </xsl:when>
                        <xsl:otherwise>
                            <ref target="#unknown"/>
                        </xsl:otherwise>
    ...
    <xsl:function name="reed:nearest-lang" as="xs:string">
        ...
        <xsl:sequence select="if (exists($langs)) then string($langs[last()]) else $default-lang"/>
    </xsl:function>
    ...
    <xsl:function name="reed:word-key" as="xs:string">
        ...
        <xsl:value-of>
            <xsl:for-each select="$w/node()">
                <xsl:choose>
                    <xsl:when test="self::ex">&#xE000;<xsl:value-of select="string(.)"/>&#xE001;</xsl:when>
                    <xsl:otherwise><xsl:value-of select="."/></xsl:otherwise>
                </xsl:choose>
            </xsl:for-each>
        </xsl:value-of>
            ...
            <xsl:for-each-group select="$instances" group-by="reed:word-key(.)">
                <xsl:map-entry key="current-grouping-key()" select="
                    map{
                        ...
                        'supplied': (some $g in current-group() satisfies $g/@supplied = 'true')
                    }"/>
            </xsl:for-each-group>
        </xsl:map>
    </xsl:function>
    ...
    <xsl:function name="reed:lang-label" as="xs:string">
        ...
        <xsl:sequence select="if (map:contains($lang-labels, $code)) then map:get($lang-labels, $code) else $code"/>
    </xsl:function>
    ...
    <xsl:function name="reed:entry-id" as="xs:string">
        ...
        <xsl:sequence select="concat('entry-', translate($lang, ':', '-'), '-', format-number($n, '0000'))"/>
    </xsl:function>
